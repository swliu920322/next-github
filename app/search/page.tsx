    ...
    <div className="flex p-4 overflow-hidden h-full">
      <div className="flex w-40 flex-col overflow-auto h-full">
        <List
          ...
          renderItem={(item) => (
            <Item className={`cursor-pointer ${item === searchParams.language && 'border-l-2 border-blue-400 active-item'}`} onClick={() => item !== searchParams.language && handleLanguage(item)}>
              {item}
            </Item>
          )}
        />
        <List
          ...
          renderItem={(item) => (
            <Item className={`cursor-pointer ${dealSort(item) && 'border-l-2 border-blue-400 active-item'}`} onClick={() => !dealSort(item) && handleOrder(item)}>
              {item.name}
            </Item>
          )}
        />
      ...
      <div className="flex-1 flex flex-col ml-4 overflow-hidden h-full relative">
        <div className="flex flex-1 flex-col gap-3 border-slate-500 border-t overflow-auto">{result?.items?.map((item) => <RepoItem key={item.id} item={item} />)}</div>
        {loading && (
